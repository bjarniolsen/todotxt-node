...
function edit(response, postData, action) {
    fs.readFile(file, "ascii", function(err, data) {
        ...
        var tasklist = data.split("\n"),
            context = tasklist[action - 1].match(/(\+[a-z.-]+)/gi), 
            project = tasklist[action - 1].match(/(\@[a-z.-]+)/gi);
            priority = tasklist[action - 1].match(/\([^\)]*\)/gi);
        ...
        var body = '<html>'+
            ...
            tasklist[action - 1]+
            '</textarea>'+
            // TODO: Select for context/project/priority
